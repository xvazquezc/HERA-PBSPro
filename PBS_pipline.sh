...
Bionano_Scaffolded_Contig=${outfolder}/Large_Contig.fasta

#the non-scaffold contigs
Bionano_NonScaffolded_Contig=${outfolder}/Small_Contig.fasta
...
cd ${outfolder}/01-Pacbio_And_NonScaffold
...
cd -
ls ${outfolder}/01-Pacbio_And_NonScaffold/*.fasta >list_Split.txt
...
perl $Working_Script/04-PBS-Qsub-Mapping2Ctg.pl ${outfolder}/list_Split.txt $Bionano_Scaffolded_Contig \
    ${outfolder}/02-Pacbio-Alignment $Working_Script $queue $genome_name ${Bwa}>log
...
cat ${outfolder}/02-Pacbio-Alignment/Part_Alignment_*.txt > ${outfolder}/02-Pacbio-Alignment/Total_Alignment.txt
#rm -f ${outfolder}/02-Pacbio-Alignment/Part_Alignment_*.txt


#Remove the pacbios and non-scaffolded contigs aligned to the internal scaffolded contigs
$Working_Script/05-Filtered_InterIncluded_Pacbio ${outfolder}/02-Pacbio-Alignment/Total_Alignment.txt \
    ${outfolder}/02-Pacbio-Alignment/InterIncluded_Pacbio.txt $InterIncluded_Identity $InterIncluded_Coverage $InterIncluded_Side


#Record the pacbio alignment of contig's head and end
$Working_Script/06-Extract_Contig_Head_Tail_Pacbio_Alignment -Align=${outfolder}/02-Pacbio-Alignment/Total_Alignment.txt \
...
$Working_Script/10-Switch_Locus_To_Positive Contig_Head_Tail_Pacbio.txt ${outfolder}/04-Graphing/Contig_Head_Tail_Pacbio_Pos.txt
...
$Working_Script/07-extract_fasta_seq_by_name ${outfolder}/02-Pacbio-Alignment/InterIncluded_Pacbio.txt ${outfolder}/Query_Merged.fasta \
    ${outfolder}/02-Pacbio-Alignment/Both_Side_Pacbio.fasta

#Split the remained pacbio or contigs into parts
cd ${outfolder}/03-Pacbio-SelfAlignment
$Working_Script/03-fasta-splitter --n-parts 30 ${outfolder}/02-Pacbio-Alignment/Both_Side_Pacbio.fasta

#Make index for every part of the pacbios and non-scaffolded contigs
cd -
ls ${outfolder}/03-Pacbio-SelfAlignment/*.fasta >${outfolder}/list_outer_pacbio.txt

perl $Working_Script/08-PBS-Qsub_job_index.pl ${outfolder}/list_outer_pacbio.txt $queue $genome_name ${Bwa} >>log
...
perl $Working_Script/09-PBS-Qsub-Pair_Alignment.pl ${outfolder}/list_outer_pacbio.txt $Working_Script $queue $genome_name ${outfolder} ${Bwa}>>log
...
#Merge all alignment into an single file
cat ${outfolder}/03-Pacbio-SelfAlignment/Part_SelfAlignment_*.txt > ${outfolder}/03-Pacbio-SelfAlignment/Total_SelfAlignment.txt

#Filter the alignment for overlaping

$Working_Script/11-PacbioAlignmentFilter ${outfolder}/03-Pacbio-SelfAlignment/Total_SelfAlignment.txt \
    $MaxOverhang_Overlap $MinIdentity_Overlap $MinOverlap_Overlap $MinExtend_Overlap > ${outfolder}/04-Graphing/PacbioAlignmentFiltered.txt

#Find the proper overlap for constructing the graph
$Working_Script/12-PacbioAlignmentLinker ${outfolder}/04-Graphing/PacbioAlignmentFiltered.txt \
    $MaxOverhang_Overlap $MinExtend_Overlap > ${outfolder}/04-Graphing/PacbioAlignmentLinked.txt
...
cat ctg_clusters.txt |sort |uniq > ${outfolder}/05-PathContig/ctg_clusters_uniq.txt
cat cluster_ori.txt |sort |uniq > ${outfolder}/05-PathContig/cluster_ori_uniq.txt

cd -


cd ${outfolder}/05-PathContig
#Make the corrected pacbios and non-scaffolded contigs into a line
$Working_Script/14-make_ctg_line ${outfolder}/05-PathContig/cluster_ori_uniq.txt cluster_ori_same_chain.txt

$Working_Script/18-compute_fasta_file_len ${outfolder}/Query_Merged.fasta Query_Len.txt

#Change the path into the same chain of bionano scaffolds
$Working_Script/15-make_junction_by_pos ${outfolder}/04-Graphing/ctg_pairs.txt Query_Len.txt cluster_ori_same_chain.txt cluster_ori_same_chain_pos.txt
...
$Working_Script/17-extract_seq_by_pos cluster_ori_same_chain_pos_for_seq.txt ${outfolder}/Query_Merged.fasta NA.fasta PathContig.fasta

#Compute the length of pathcontigs
$Working_Script/18-compute_fasta_file_len PathContig.fasta ${outfolder}/06-Daligner/PathContig_Len.txt
...
cd ${outfolder}/10-Contig_Pairs
$Working_Script/Check
touch overlap.txt

#formating the contig pairs based on the paths
$Working_Script/03-Formate_Contig_Pairs_By_Paths overlap.txt ${outfolder}/05-PathContig/ctg_clusters_uniq.txt Contig_Pairs.txt
...
cd ${outfolder}/06-Daligner

#extract the paths which connects the final selected contigs
$Working_Script/19-Path2Scaffold_NoBioNano ${outfolder}/10-Contig_Pairs/Selected_Pairs.txt \
    ${outfolder}/05-PathContig/ctg_clusters_uniq.txt PathContig_Len.txt Path_Scaffold.txt

#rename the path contigs
$Working_Script/20-PathContig-Rename_NoBioNano Path_Scaffold.txt ${outfolder}/05-PathContig/PathContig.fasta PathContig_Rename.fasta >log

$Working_Script/Rename1 ${outfolder}/10-Contig_Pairs/SuperContig.fasta  SuperContig_Rename.fasta >Rename_Pairs.txt
...
perl $Working_Script/21-PBS-Daligner_New.pl ${outfolder}/06-Daligner/Scaffold2Ctg_Gap.txt  \
    ${outfolder}/06-Daligner/Prosudo_ScaffoldNonEnzyme2Contig.fasta \
    ${outfolder}/06-Daligner/PathContig_Rename.fasta $queue $Working_Script $genome_name $DAZZ_DB $DALIGNER ${outfolder}
